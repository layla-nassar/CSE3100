...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    if (pid == -1) {
        ...
        die("execvp() failed."); // If execvp fails, exit
    } else {
...
int wait_on_program(Program *prog)
{
    ...
    return -1;  // In case the process did not exit normally
}
...
void prepare_pipes(Program *programs, int num_programs)
{
    for (int i = 0; i < num_programs - 1; i++) {
        ...
        programs[i].fd_out = pipefds[PIPEFD_WRITE];  // stdout of current program
        programs[i + 1].fd_in = pipefds[PIPEFD_READ]; // stdin of next program
    }
}
