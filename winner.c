...
void* player_func(void* threadarg) {
	...
	pthread_rwlock_wrlock(&rwlock); // add one line of code below
	scores[count].id = my_id;
	scores[count].score = score;
	count++;
	pthread_rwlock_unlock(&rwlock); // add one line of code below

	pthread_barrier_wait(&barrier); // wait for all threads to get here

	pthread_rwlock_rdlock(&rwlock); // add one line of code below
