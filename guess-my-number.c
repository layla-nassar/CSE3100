...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    if (write(fdc[PIPEFD_WRITE], &max_value, sizeof(max_value)) == -1)
        die("Failed to send max value");

    int guess;
    int result;

    do {
        if (read(fdp[PIPEFD_READ], &guess, sizeof(guess)) == -1)
            die("Failed to read guess");

        result = gmn_check(&gmn, guess);

        if (write(fdc[PIPEFD_WRITE], &result, sizeof(result)) == -1)
            die("Failed to send result");

    } while (result != 0);

    char *final_message = gmn_get_message(&gmn);
    if (write(fdc[PIPEFD_WRITE], final_message, strlen(final_message)) == -1)
        die("Failed to send final message");
...
int main(int argc, char *argv[])
{
    ...
    int max;
    if (read(fdc[PIPEFD_READ], &max, sizeof(max)) == -1)
        die("Failed to read max value from child");
    ...
    do { 
        ...
        if (write(fdp[PIPEFD_WRITE], &guess, sizeof(guess)) == -1)
            die("Failed to send guess to child");

        if (read(fdc[PIPEFD_READ], &result, sizeof(result)) == -1)
    ...
    char final_message[MSG_BUF_SIZE] = {0}; 
    int i = 0;
    do {
        if (read(fdc[PIPEFD_READ], &final_message[i], 1) == -1)
            die("Failed to read final message from child");
    } while (final_message[i++] != '\n');
